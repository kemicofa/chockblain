...
class Blockchain<T> {
  ...
  private buildHash({ payload, index, prevHash }: Omit<Block<T>, 'hash'>) {
    ...
    if(payload instanceof Uint8Array) {
        ...
        formattedDataToHash = new TextEncoder().encode(JSON.stringify(payload) + suffix)
    } else {
        throw new Error('Unrecognized payload format; expected Uint8Array, string or object');
    }
  ...
  validateIntegrity() {
    ...
    for(let i = 0; i < this.blocks.length; i++) {
        ...
        if(prevHash !== currentBlock.prevHash) {
            throw new Error(`Previous hash ${currentBlock.prevHash} did not match expected value ${prevHash}.`);
        }
        ...
        if(currentHash !== currentBlock.hash) {
            throw new Error(`Expected hash ${currentHash} but got ${currentBlock.hash} at index ${i}`)
        }
